...
void CCarTree::loadTree()
{
	...
	tvstruct.item.iImage = TO_ID(IDB_OBJECT);
	tvstruct.item.iSelectedImage = TO_ID(IDB_OBJECT);
	tvstruct.item.pszText = _T("All Objects");
...
int CCarTree::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (!m_CarTree.CreateEx(WS_EX_CLIENTEDGE,TVS_HASLINES  | TVS_HASBUTTONS | TVS_SHOWSELALWAYS | TVS_DISABLEDRAGDROP | WS_VISIBLE | WS_CHILD | WS_BORDER ,r2,this,IDC_CARTREE))
       return -1;
...
{
	pImageList = (((CObjectEditorApp*)AfxGetApp())->getSharedImageList());
...
void CCarTree::Refresh(GPOObject *object)
{
   if (object->carNode) m_CarTree.DeleteItem(object->carNode);
   if (object!=NULL) object->LoadTree(this);
}
	...
	if (m_CarTree)
	{
	 ...
	 if (dlg != NULL)
	 {
		...
		if (dlg->getObject())
		{
		 ...
		 dlg->getObject()->updateData(FALSE,FALSE);
